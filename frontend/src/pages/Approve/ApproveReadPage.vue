  ...
  methods: {
    ...
    async fetchApprove() {
      try {
        ...
        const approveResponse = await axios.get(
          `http://www.hello-r-loha.kro.kr/api/approve/read/${this.id}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: "Bearer " + token,
    ...
    async fetchApproveLine(approveId) {
      try {
        ...
        const response = await axios.get(
          `http://www.hello-r-loha.kro.kr/api/approve/line/2/${approveId}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: "Bearer " + token,
    ...
    async deleteApprove() {
      if (confirm("정말로 이 결재를 회수  하시겠습니까?")) {
        try {
          const token = sessionStorage.getItem("token");
          await axios.delete(`http://www.hello-r-loha.kro.kr/api/approve/cancel/${this.id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + token,
        },
