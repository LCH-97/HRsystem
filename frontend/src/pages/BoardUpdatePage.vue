...
export default {
  ...
  methods: {
    ...
    updateBoard() {
      ...
      axios
        .patch(`http://localhost:8080/board/update`, formData, { headers })
        .then(() => {
    ...
    fetchFiles() {
      axios
        .get(`http://localhost:8080/board/files/${this.board.id}`)
        .then((response) => {
    ...
    deleteFile(fileId) {
      axios
        .delete(`http://localhost:8080/board/files/delete/${fileId}`)
        .then(() => {
